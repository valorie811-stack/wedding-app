...
export async function saveMoodItem(input) {
  ...
  const row = {
    ...
    board: input.board?.trim() || "General",
  ...
  const res = isSeed(input.id)
    ? await supabase.from("moodboard_items").insert(row).select("id").maybeSingle()
    : await supabase.from("moodboard_items").update(row).eq("id", input.id).select("id").maybeSingle();
