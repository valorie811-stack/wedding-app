...
async function gather() {
  ...
  if (admin) {
    try {
      ...
      // All three, not just the RSVP query. Checking `r` alone let a failed
      // tasks read fall through to `(t.data || [])`, and the digest went out
      // reporting "No open tasks" — the same bug lib/data.js closed with its
      // `failed()` helper.
      const bad = [r, t, e].find((q) => q?.error);
      if (bad) {
        console.error(
          `[digest] Supabase query failed, falling back to sample data — ${bad.error.message}`
        );
      } else {
        return {
...
export async function buildDigestHtml() {
  ...
  // Compared as calendar-day strings, which is what `due` and `event_date`
  // already are. `new Date("2027-10-08") >= new Date()` measures a UTC midnight
  // against the current instant, so anything due *today* was always in the
  // past by the time the cron ran and never made the digest.
  ...
  const upcomingTasks = tasks
    .filter((t) => t.status !== "done" && t.due && String(t.due) >= todayStr)
  ...
  const taskRows =
    upcomingTasks.map((t) => `<li style="margin:4px 0;color:${C.body}">${esc(t.title)} — <span style="color:${C.sub}">${esc(fmt(t.due))}${t.code ? ` · ${esc(t.code)}` : ""}</span></li>`).join("") ||
    `<li style="color:${C.muted}">No open tasks 🎉</li>`;

  const eventRows =
    nextEvents.map((e) => `<li style="margin:4px 0;color:${C.body}">${esc(e.name)} — <span style="color:${C.sub}">${esc(fmt(e.date))} · ${esc(e.code)}</span></li>`).join("") ||
