...
export function aggregate({ events = [], budget = [], tasks = [], rsvp = [], vendors = [], guests = [] }, scope = "BOTH") {
  ...
  // is given. `heads` is attached upstream by lib/data.js; the fallback covers
  // a reply whose guest row did not come back with it.
  ...
  rsvp
    ...
    .forEach((r) => {
      ...
      if (r.status === "confirmed") confirmedHeads += r.heads ?? 1;
    });
