...
export async function setupPin(pin) {
  ...
  // A stable code, not error.message. These flow straight into a `login.err.*`
  // lookup, so a raw Postgres string both failed to translate and printed
  // database internals on an unauthenticated screen. The detail stays in the
  // server log where it is useful.
  if (error) {
    console.error("[auth] setupPin failed:", error.message);
...
export async function changePin(current, next) {
  ...
  const { error } = await admin
    ...
    .eq("id", 1);
  if (error) {
    console.error("[auth] changePin failed:", error.message);
    return { ok: false, error: "save-failed" };
  }
  return { ok: true };
}
