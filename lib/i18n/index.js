...
// Resolve a key built from runtime data, falling back when it is not in the
// dictionary. `t` returns the key path on a miss and a key path is a non-empty
// string, so the obvious `t(key) || t(fallback)` never reaches its fallback:
// the login screen rendered "login.err.<raw Postgres message>" on screen. The
// miss has to be detected by comparing the result against the key.
export function translateOr(t, key, fallbackKey) {
  const label = t(key);
