...
// Every value that reaches an email template comes from a free-text field the
// owner typed — a task title, a vendor name, an assignee. None of it is HTML,
// and a task called "Cake tasting <2pm>" silently lost its suffix in the
// rendered mail because the templates interpolated it raw. Not an attack path
// on a single-owner app, but the output was wrong.
...
// `title` is plain text and is escaped; `bodyHtml` is markup by contract and
// is the caller's job to escape.
export function emailShell(title, bodyHtml) {
  return `<!doctype html><html><body style="margin:0;background:${C.page};font-family:Helvetica,Arial,sans-serif">
    <div style="max-width:560px;margin:0 auto;padding:24px">
      ...
      <h1 style="font-size:20px;color:${C.ink};margin:4px 0 14px">${escapeHtml(title)}</h1>
