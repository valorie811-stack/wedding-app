...
// Current signed-in member (role + name). Null when not configured / signed out.
export async function getCurrentMember() {
  ...
  return {
    id: user.id,
    email: user.email,
    full_name: member?.full_name || user.email,
...
export async function getTasksData() {
  ...
  if (supabase) {
    try {
      const { data, error } = await supabase
        .from("tasks")
        .select("id,title,due_date,status,assignee,wedding:weddings(code)")
    ...
    try {
      const [events, tasks] = await Promise.all([
        supabase
          .from("events")
          .select(
            "id,name_en,name_vi,name_zh,event_date,start_time,end_time,location,event_type,is_halal,wedding:weddings(code)"
          ),
        supabase.from("tasks").select("id,title,due_date,status,assignee,wedding:weddings(code)"),
      ]);
    ...
    try {
      const [eventsRes, guestsRes] = await Promise.all([
        ...
        supabase
          .from("guests")
          .select(
            "id,full_name,email,phone,side,plus_one,dietary,notes,guest_events(event_id,rsvp_status)"
          )
...
// Settings & Access — member roster + pending invites + the viewer's own role.
  ...
  if (supabase) {
    try {
      ...
      if (!mem.error && mem.data?.length) {
        return {
          members: mem.data.map((m) => ({ id: m.id, full_name: m.full_name, role: m.role, locale: m.locale })),
          invites: (inv.data || []).map((i) => ({ id: i.id, email: i.email, role: i.role, created_at: i.created_at })),
          currentRole: me?.role || "family",
          preview: false,
        };
      }
    } catch {
