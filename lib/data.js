...
// What a wedding is contractually on the hook for, per category.
//
// A vendor's total_cost is a commitment, not a payment, so it is deliberately
// kept out of "spent" — folding it in would overstate what has left the bank.
// But it is not nothing either: a signed RM 25,000 venue against a hand-set
// RM 10,000 Venue budget is a problem the planner should see before the
// invoice lands, and comparing planned against actual alone never shows it.
export function vendorCommitments(vendors = []) {
  const map = new Map();
  vendors
    .filter((v) => v.contract_status !== "cancelled" && Number(v.total_cost) > 0)
    .forEach((v) => {
      const key = `${v.code}|${v.category || VENDOR_FALLBACK_CATEGORY}`;
      ...
      row.committed += Number(v.total_cost) || 0;
      row.paid += Number(v.deposit_paid) || 0;
      map.set(key, row);
    });
  // Outstanding is what is still owed under signed contracts — the number that
  // decides whether the remaining budget actually covers what is coming.
  return [...map.values()].map((r) => ({ ...r, outstanding: Math.max(0, r.committed - r.paid) }));
}
...
// Seed events carry their wedding code directly; guest_events reference them by
// id, so this maps one to the other for the guest invite split.
const seedCodeByEventId = new Map(SEED_EVENTS.map((e) => [e.id, e.code]));
...
function seedSource() {
  return {
    ...
    vendors: SEED_VENDORS.map((v) => ({
      ...
      halalCertified: v.is_halal_certified,
    })),
    guests: SEED_GUESTS.map((g) => ({
      dietary: g.dietary || [],
      codes: splitInvites(
        SEED_GUEST_EVENTS.filter((ge) => ge.guest_id === g.id).map((ge) => ({
          code: seedCodeByEventId.get(ge.event_id),
          status: ge.status,
        }))
      ).codes,
    })),
...
async function dbSource(supabase) {
  try {
    const [events, budgetCats, budgetItems, tasks, rsvp, vendors, guests] = await Promise.all([
      supabase
        .from("events")
      ...
      supabase
        .from("guest_events")
        // The guest join is only here for the head count: a reply is one reply
        // whatever the size of the party behind it, but the dashboard reports
        // both, and party size lives on the guest row.
        .select(
          "rsvp_status,guest:guests(id,party_size,plus_one),event:events(wedding:weddings(code))"
        ),
      supabase
        .from("vendors")
        .select(
          "id,name,category,contract_status,deposit_paid,is_halal_certified,wedding:weddings(code,currency)"
        ),
      // Dietary needs and which weddings a guest is actually invited to, for
      // the halal readiness check below.
      supabase
        .from("guests")
        .select("dietary,guest_events(rsvp_status,event:events(wedding:weddings(code)))"),
    ]);

    if (failed("dashboard", events, budgetCats, budgetItems, tasks, rsvp, vendors, guests)) {
      return null;
    }
    ...
    return {
      ...
      rsvp: (rsvp.data || []).map((r) => ({
        ...
        heads: headcount(one(r.guest)),
        guestId: one(r.guest)?.id ?? null,
      })),
      ...
      guests: (guests.data || []).map((g) => ({
        dietary: g.dietary || [],
        codes: splitInvites(
          (g.guest_events || []).map((ge) => ({
            code: one(one(ge.event)?.wedding)?.code,
...
export async function getBudgetData() {
  ...
  if (supabase) {
    try {
      const [cats, items, vendors] = await Promise.all([
        ...
        supabase
          .from("vendors")
          .select("id,name,category,contract_status,total_cost,deposit_paid,wedding:weddings(code,currency)")
          .order("created_at", { ascending: true }),
      ]);
      if (!failed("budget", cats, items, vendors)) {
        ...
        return {
          ...
          items: [
            ...
            ...vendorPaymentItems(vendorRows),
          ],
          commitments: vendorCommitments(vendorRows),
          preview: false,
        };
      }
    } catch {
  ...
  return {
    ...
    commitments: vendorCommitments(SEED_VENDORS),
    preview: true,
  };
}
...
export async function getTasksData() {
  ...
  if (supabase) {
    try {
      // Vendors and events come along so the board can name what a task is
      // about, and so its form can offer the choices, rather than leaving the
      // link as an id the user never sees.
      const [res, vend, evs] = await Promise.all([
        supabase
          .from("tasks")
          .select(
            "id,title,due_date,status,assignee,recur_freq,recur_until,remind_days_before,vendor_id,event_id,wedding:weddings(code)"
          )
          .order("due_date", { ascending: true }),
        supabase.from("vendors").select("id,name,wedding:weddings(code)").order("name"),
        supabase
          .from("events")
          .select("id,name_en,event_date,wedding:weddings(code)")
          .order("event_date"),
      ]);
      if (!failed("tasks", res, vend, evs)) {
        return {
          ...
          vendors: (vend.data || []).map((v) => ({
            id: v.id,
            code: one(v.wedding)?.code,
            name: v.name,
          })),
          events: (evs.data || []).map((e) => ({
            id: e.id,
            code: one(e.wedding)?.code,
            name: e.name_en,
            date: e.event_date,
  ...
  return {
    tasks: SEED_TASKS,
    vendors: SEED_VENDORS.map((v) => ({ id: v.id, code: v.code, name: v.name })),
    events: SEED_EVENTS.map((e) => ({ id: e.id, code: e.code, name: e.name.en, date: e.date })),
    preview: true,
  };
}
    ...
    try {
      const [eventsRes, guestsRes] = await Promise.all([
        ...
        supabase
          .from("guests")
          .select(
            "id,full_name,side,plus_one,plus_one_name,party_size,dietary,notes,country,category,invite_or_not,guest_events(event_id,rsvp_status)"
          )
...
// Splits a guest's invitations into the weddings they may still attend and the
// ones they have turned down.
//
// Seating a guest who declined every event at a wedding holds a seat nobody
// will sit in, and the seat count is the whole point of the planner. Pending is
// deliberately treated as attending: seating provisional guests is normal, and
// a table can be rearranged later — a declined guest is the only unambiguous
// "not coming".
//
// `invites` is [{ code, status }]; the same shape is built from Supabase rows
// and from seed data so both paths behave identically.
export function splitInvites(invites = []) {
  const byCode = new Map();
  invites.forEach(({ code, status }) => {
    if (!code) return;
    const seen = byCode.get(code) || { any: false, attending: false };
    seen.any = true;
    if (status !== "declined") seen.attending = true;
    byCode.set(code, seen);
  });
  const codes = [];
  const declinedCodes = [];
  byCode.forEach((v, code) => (v.attending ? codes : declinedCodes).push(code));
  return { codes, declinedCodes };
}
    ...
    try {
      const [tbls, asg, gs] = await Promise.all([
        ...
        supabase
          .from("guests")
          .select(
            "id,full_name,plus_one,plus_one_name,party_size,guest_events(rsvp_status,event:events(wedding:weddings(code)))"
          )
          .order("full_name", { ascending: true }),
      ]);
      if (!failed("seating", tbls, asg, gs)) {
        return {
          ...
          guests: (gs.data || []).map((g) => ({
            id: g.id,
            full_name: g.full_name,
            // Seating shows the whole party on one chip and counts every head
            // in it against the table's capacity — see headcount().
            plus_one: g.plus_one,
            plus_one_name: g.plus_one_name,
            party_size: g.party_size,
            ...splitInvites(
