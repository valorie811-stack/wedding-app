    ...
    try {
      const [eventsRes, guestsRes] = await Promise.all([
        ...
        supabase
          .from("guests")
          .select(
            "id,full_name,side,plus_one,plus_one_name,dietary,notes,country,category,invite_or_not,guest_events(event_id,rsvp_status)"
          )
    ...
    try {
      const [tbls, asg, gs] = await Promise.all([
        ...
        supabase
          .from("guests")
          .select(
            "id,full_name,plus_one,plus_one_name,guest_events(rsvp_status,event:events(wedding:weddings(code)))"
          )
          .order("full_name", { ascending: true }),
      ]);
